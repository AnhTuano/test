        ...
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 relative overflow-hidden group">
            <div className="flex justify-between items-center mb-6">
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        <h3 className="text-lg font-bold text-slate-800">Lưu lượng truy cập</h3>
            ...
            <div className="w-full aspect-[2/1] md:aspect-[4/1] max-h-[250px]">
                <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full overflow-visible" preserveAspectRatio="none">
                    ...
                    {[0, 0.25, 0.5, 0.75, 1].map((tick, i) => (
                        <line key={i} x1={padding} y1={height - padding - (tick * (height - 2 * padding))} x2={width - padding} y2={height - padding - (tick * (height - 2 * padding))} stroke="currentColor" className="text-slate-100" strokeWidth="1" strokeDasharray="4 4" />
                    ))}
                    ...
                    {activityData.map((d, i) => {
                         ...
                         return (
                             <g key={i} className="group/point">
                                 <circle cx={x} cy={y} r="4" className="fill-blue-600 stroke-white stroke-2 transition-all group-hover/point:r-6" />
                                 <g className="opacity-0 group-hover/point:opacity-100 transition-opacity pointer-events-none">
                                     <rect x={x - 20} y={y - 40} width="40" height="25" rx="6" className="fill-slate-800" />
                                     <text x={x} y={y - 24} textAnchor="middle" className="fill-white text-[10px] font-bold">{d.value}</text>
                                 </g>
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="flex justify-end">
             <button onClick={fetchData} disabled={loading} className="flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-slate-600 bg-slate-100 hover:bg-blue-50 hover:text-blue-600 rounded-lg transition-all active:scale-95">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-6">
        {[
            { label: 'Thời gian vận hành', value: metrics.uptime, icon: Activity, color: 'text-emerald-600', bg: 'bg-emerald-100' },
            { label: 'Bộ nhớ RAM', value: metrics.memoryUsage, icon: Server, color: 'text-blue-600', bg: 'bg-blue-100' },
            { label: 'Kết nối hiện hành', value: dbStats.activeConnections, icon: Database, color: 'text-purple-600', bg: 'bg-purple-100' }
        ].map((stat, i) => (
        ...
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                ...
                <div className="flex gap-2">
                    <button onClick={exportData} className="text-xs font-bold text-blue-600 hover:bg-blue-50 px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1 border border-blue-200 active:scale-95">
                        <Download className="w-3.5 h-3.5" /> Xuất Excel
                    </button>
                    <button onClick={handleClearHistory} className="text-xs font-bold text-red-500 hover:bg-red-50 px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1 border border-red-200 active:scale-95">
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {loginHistory.map((log: any) => (
                            <tr key={log.id} className="hover:bg-slate-50 transition-colors">
                                ...
                                <td className="px-6 py-4">
                                    <div className="text-sm text-slate-600 font-medium">{new Date(log.timestamp).toLocaleTimeString('vi-VN')}</div>
