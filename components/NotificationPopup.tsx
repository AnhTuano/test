...
const NotificationPopup: React.FC<NotificationPopupProps> = ({ notification, onClose, isPreview = false }) => {
  ...
  const getColors = () => {
    switch (notification.type) {
      case 'warning': return { bg: 'bg-amber-50', border: 'border-amber-100', btn: 'bg-amber-600 hover:bg-amber-700' };
      case 'error': return { bg: 'bg-red-50', border: 'border-red-100', btn: 'bg-red-600 hover:bg-red-700' };
      case 'success': return { bg: 'bg-emerald-50', border: 'border-emerald-100', btn: 'bg-emerald-600 hover:bg-emerald-700' };
      default: return { bg: 'bg-blue-50', border: 'border-blue-100', btn: 'bg-blue-600 hover:bg-blue-700' };
    }
  };
  ...
  return (
    <div className={`${positionClasses} flex items-center justify-center p-4 ${backdropClasses}`}>
      <div className={`w-full max-w-lg bg-white rounded-3xl shadow-2xl overflow-hidden relative border ${colors.border} ${modalAnimation}`}>
