...
const UsersTab: React.FC = () => {
  ...
  if (loading && users.length === 0) return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      {/* User Rows Skeleton */}
      <div className="bg-white rounded-[2rem] border border-slate-200 p-4 space-y-2">
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      ...
      {/* Stats Cards */}
      <div className="grid grid-cols-3 gap-3 md:gap-6">
        <div className="bg-white p-5 rounded-[2rem] border border-slate-200 text-center md:text-left shadow-sm">
           <div className="text-[10px] md:text-xs font-extrabold text-slate-400 uppercase tracking-widest">Tổng User</div>
           <div className="text-2xl md:text-4xl font-black text-slate-900 mt-2 tracking-tight">{stats.total}</div>
        </div>
        <div className="bg-white p-5 rounded-[2rem] border border-slate-200 text-center md:text-left shadow-sm">
      ...
      {/* Filters */}
      <div className="flex flex-col xl:flex-row gap-3 bg-white p-4 rounded-[2rem] border border-slate-200 shadow-sm">
        <div className="relative flex-1">
           <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" />
           <input type="text" placeholder="Tìm kiếm tên, mã SV..." className="w-full pl-12 pr-4 py-3.5 bg-slate-50 border border-transparent rounded-2xl text-sm font-semibold text-slate-900 focus:bg-white focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all placeholder:text-slate-400" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        </div>
        <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative min-w-[150px]">
                <Filter className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                <select value={filterStatus} onChange={e => setFilterStatus(e.target.value as any)} className="w-full pl-10 pr-10 py-3.5 bg-slate-50 border border-transparent rounded-2xl text-sm font-bold text-slate-700 appearance-none outline-none focus:bg-white focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all cursor-pointer">
      ...
      <div className="bg-transparent md:bg-white md: md:rounded-[2.5rem] md:border md:border-slate-200 md: md:shadow-sm overflow-hidden">
            ...
            <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100">
                    {paginatedUsers.map(user => (
                        <tr key={user.id} onClick={() => setSelectedUserForDetail(user.username)} className="hover:bg-blue-50/50 transition-colors group cursor-pointer">
                            
                            {/* Column 1: Identity */}
                            <td className="px-8 py-5">
                                <div className="flex items-center gap-4">
                                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-base font-bold shadow-sm shrink-0 transition-transform group-hover:scale-110 ${user.role === 'ADMIN' ? 'bg-slate-800 text-white' : 'bg-white border border-slate-100 text-slate-600'}`}>
                                        {user.full_name.charAt(0)}
                                    </div>
                                    <div className="min-w-0">
                                        <div className="font-bold text-slate-900 text-base group-hover:text-blue-600 transition-colors truncate max-w-[200px]">
                                            {user.full_name}
                                        </div>
                            ...
                            <td className="px-6 py-5">
                                <div className="space-y-2">
                                    <div className="flex flex-wrap gap-2">
                                        {user.student_code ? (
                                            <span className="font-mono bg-slate-100 text-slate-600 px-2 py-1 rounded-lg text-xs font-bold border border-slate-200">
                                                {user.student_code}
                                            </span>
                                        ) : (
                            ...
                            <td className="px-6 py-5 text-center">
                                {user.role === 'ADMIN' ? (
                                    ...
                                    <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold bg-slate-100 text-slate-500 border border-slate-200">
                            ...
                            <td className="px-8 py-5 text-right">
                                <div className="flex items-center justify-end gap-4">
                                    <span className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold border ${user.status === 'active' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-red-50 text-red-600 border-red-100'}`}>
                                    ...
                                    <button 
                                        ...
                                        className={`p-2.5 rounded-xl transition-all border active:scale-95 ${
                                            user.status === 'active' 
                                            ? 'border-slate-200 bg-white text-slate-400 hover:text-red-500 hover:border-red-200 hover:bg-red-50' 
        ...
        <div className="md:hidden space-y-4">
            {paginatedUsers.map(user => (
                <div key={user.id} onClick={() => setSelectedUserForDetail(user.username)} className="bg-white rounded-[2rem] shadow-sm border border-slate-100 active:scale-[0.98] transition-all touch-manipulation overflow-hidden">
                    
                    {/* Card Header */}
                    <div className="p-5 flex items-center gap-4 border-b border-slate-50">
                        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-lg font-bold shadow-sm shrink-0 ${user.role === 'ADMIN' ? 'bg-slate-800 text-white' : 'bg-slate-50 text-slate-600'}`}>
                            {user.full_name.charAt(0)}
                        </div>
                        <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                                <h4 className="font-bold text-slate-900 text-lg truncate">{user.full_name}</h4>
                    ...
                    <div className="p-5 bg-slate-50/50 grid grid-cols-2 gap-4">
                         <div className="space-y-1.5">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {user.student_code ? (
                                    <span className="font-mono bg-white text-slate-600 px-2 py-1 rounded-lg text-xs font-bold border border-slate-200">
                                        {user.student_code}
                                    </span>
                    ...
                    <div className="p-4 border-t border-slate-100 flex items-center justify-between bg-white">
                         <div className={`font-bold text-xs flex items-center gap-1.5 px-3 py-1.5 rounded-full ${user.status === 'active' ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'}`}>
        ...
        {totalPages > 1 && (
            <div className="px-6 py-6 border-t border-slate-100 flex justify-center items-center gap-4 bg-slate-50/50">
                <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="p-2.5 rounded-xl bg-white border border-slate-200 hover:bg-slate-50 disabled:opacity-30 transition-colors shadow-sm"><ChevronLeft className="w-5 h-5" /></button>
                <span className="text-sm font-bold text-slate-600">Trang {currentPage} / {totalPages}</span>
                <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="p-2.5 rounded-xl bg-white border border-slate-200 hover:bg-slate-50 disabled:opacity-30 transition-colors shadow-sm"><ChevronRight className="w-5 h-5" /></button>
            </div>
        )}
      </div>
    </div>
