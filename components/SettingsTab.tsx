...
const SettingInput = memo(({ label, value, onChange, icon: Icon, placeholder, id }: SettingInputProps) => (
  <div className="space-y-2">
     <label htmlFor={id} className="text-xs font-bold text-slate-500 uppercase tracking-wider ml-1">{label}</label>
     <div className="relative group">
        ...
        <input 
          ...
          value={value} 
          onChange={onChange} 
          className={`w-full ${Icon ? 'pl-12' : 'pl-4'} pr-4 py-3.5 bg-slate-50 hover:bg-slate-100 border border-transparent rounded-2xl focus:bg-white focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all font-semibold text-slate-900 placeholder:text-slate-400 shadow-sm`}
...
const SettingsTab: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
        ...
        {/* General Config */}
        <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm p-6 lg:p-8 flex flex-col h-full">
            ...
            <div className="space-y-6 flex-1">
                ...
                <div className="mt-6 pt-6 border-t border-slate-100">
                    <div className="flex justify-between items-center group cursor-pointer" onClick={() => setSettings({...settings, maintenanceMode: !settings.maintenanceMode})}>
                        <div>
                            <label className="text-sm font-bold text-slate-900 group-hover:text-blue-600 transition-colors">Chế độ bảo trì</label>
                            <p className="text-xs text-slate-500">Chỉ Admin mới có thể đăng nhập</p>
                        </div>
                        <div className={`w-14 h-8 rounded-full p-1 transition-colors duration-300 ${settings.maintenanceMode ? 'bg-blue-600' : 'bg-slate-200'}`}>
            ...
            <div className="space-y-6 flex-1">
                <div className="bg-slate-50 rounded-2xl p-5 border border-slate-100">
                    <div className="flex justify-between items-center cursor-pointer" onClick={() => setSettings({...settings, disableStandardLogin: !settings.disableStandardLogin})}>
                        <div>
                            <label className="text-sm font-bold text-slate-900">Tắt đăng nhập thường</label>
                            <p className="text-xs text-slate-500">Chỉ cho phép đăng nhập qua SSO (Google/Microsoft)</p>
                        </div>
                        <div className={`w-14 h-8 rounded-full p-1 transition-colors duration-300 ${settings.disableStandardLogin ? 'bg-purple-600' : 'bg-slate-200'}`}>
                ...
                {settings.disableStandardLogin && (
                    <div className="space-y-3 animate-in fade-in slide-in-from-top-2">
                        <div className="flex justify-between items-center px-1">
                             <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Thông báo hiển thị</label>
                             <div className="flex gap-2">
                                <button onClick={() => insertFormatting('<b>', '</b>')} className="px-2 py-1 bg-slate-100 hover:bg-slate-200 rounded-lg text-xs font-bold transition-colors">B</button>
                                <button onClick={() => insertFormatting('<i>', '</i>')} className="px-2 py-1 bg-slate-100 hover:bg-slate-200 rounded-lg text-xs italic transition-colors">I</button>
                                <button onClick={() => insertFormatting('<br/>', '')} className="px-2 py-1 bg-slate-100 hover:bg-slate-200 rounded-lg text-xs transition-colors">↵</button>
                             </div>
                        </div>
                        <textarea 
                            ref={messageInputRef} 
                            ...
                            className="w-full px-5 py-4 bg-amber-50 border border-amber-100 rounded-2xl focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500 outline-none text-sm font-medium text-slate-700 transition-all resize-none" 
                        />
                        <div className="flex items-start gap-2 text-xs text-amber-600 px-2">
        ...
        {/* Floating Action Bar */}
        <div className="sticky bottom-6 z-40 bg-white/90 backdrop-blur-xl p-2 rounded-[2rem] border border-slate-200 shadow-2xl shadow-slate-200/50 flex items-center justify-between max-w-2xl mx-auto">
            <button 
                onClick={handleFactoryReset} 
                className="px-6 py-3 text-xs font-bold text-red-500 hover:bg-red-50 rounded-full transition-colors"
            >
                Khôi phục mặc định
            </button>
            <button 
                ...
                className="px-8 py-3 bg-slate-900 hover:bg-blue-600 text-white font-bold rounded-[1.5rem] shadow-lg transition-all active:scale-95 flex items-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
            >
